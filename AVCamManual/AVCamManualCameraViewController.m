...
static NSArray<NSNumber *> * exposureModes = @[@(AVCaptureExposureModeContinuousAutoExposure), @(AVCaptureExposureModeLocked), @(AVCaptureExposureModeCustom)];
...
// TO-DO:
// Write a block that locks the camera for configuration...
// ...executes another block passed to it that configures a camera setting...
// ...then unlocks the camera

static

static void (^configureCameraSetting)(AVCaptureDevice *, void(^)(void)) = ^ void (AVCaptureDevice * video_device, void (^__strong cameraSetting)(void)) {
    dispatch_async(dispatch_get_main_queue(), ^{
        __autoreleasing NSError * error = nil;
        if ([video_device lockForConfiguration:&error])
        {
            cameraSetting();
            [video_device unlockForConfiguration];
        } else {
            NSLog(@"Error configuring video device: %@", error.localizedDescription);
        }
    });
};

static void (^setExposureISOSettings)(AVCaptureDevice *, UISegmentedControl *, UISlider *, UISlider *) = ^ (AVCaptureDevice * video_device, UISegmentedControl * exposure_mode_control, UISlider * exposure_duration_slider, UISlider * iso_slider) {
    dispatch_async(dispatch_get_main_queue(), ^{
        [exposure_mode_control setSelectedSegmentIndex:[exposureModes indexOfObject:@(video_device.exposureMode)]];
        exposure_duration_slider.enabled = (video_device.exposureMode == AVCaptureExposureModeCustom);
        iso_slider.enabled = (video_device.exposureMode == AVCaptureExposureModeCustom);
        double exposureDurationSeconds = CMTimeGetSeconds(video_device.exposureDuration);
        double minExposureDurationSeconds = MAX(CMTimeGetSeconds(video_device.activeFormat.minExposureDuration), kExposureMinimumDuration);
        double maxExposureDurationSeconds = CMTimeGetSeconds(video_device.activeFormat.maxExposureDuration);
        ...
        exposure_duration_slider.value = pow(p, 1 / kExposureDurationPower);
        iso_slider.value = video_device.ISO;
    });
};

static void (^defaultExposureISOSettings)(AVCaptureDevice *, UISegmentedControl *, UISlider *, UISlider *) = ^ (AVCaptureDevice * video_device, UISegmentedControl * exposure_mode_segmented_control, UISlider * exposure_duration_slider, UISlider * iso_slider) {
    dispatch_async(dispatch_get_main_queue(), ^{
        [exposure_mode_segmented_control setSelectedSegmentIndex:0];
        [video_device setExposureMode:AVCaptureExposureModeContinuousAutoExposure];
        setExposureISOSettings(video_device, exposure_mode_segmented_control, exposure_duration_slider, iso_slider);
    });
};

static void (^initExposureISOSettings)(AVCaptureDevice *, UISegmentedControl *, UISlider *, UISlider *) = ^ (AVCaptureDevice * video_device, UISegmentedControl * exposure_mode_segmented_control, UISlider * exposure_duration_slider, UISlider * iso_slider) {
    dispatch_async(dispatch_get_main_queue(), ^{
        exposure_duration_slider.minimumValue = 0;
        exposure_duration_slider.maximumValue = 1;
        exposure_duration_slider.enabled = (video_device && video_device.exposureMode == AVCaptureExposureModeCustom);
        iso_slider.minimumValue = video_device.activeFormat.minISO;
        iso_slider.maximumValue = video_device.activeFormat.maxISO;
        iso_slider.enabled = (video_device.exposureMode == AVCaptureExposureModeCustom);
        defaultExposureISOSettings(video_device, exposure_mode_segmented_control, exposure_duration_slider, iso_slider);
    });
};

- (IBAction)changeExposureMode:(id)sender
{
    setExposureISOSettings(self.videoDevice, self.exposureModeControl, self.exposureDurationSlider, self.ISOSlider);
...
{
    configureCameraSetting(self.videoDevice, ^ (void) {
        ^ (AVCaptureDevice * video_device, UISegmentedControl * exposure_mode_segmented_control, UISlider * exposure_duration_slider, UISlider * iso_slider) {
            return ^ (void) {
                initExposureISOSettings(video_device, exposure_mode_segmented_control, exposure_duration_slider, iso_slider);
            };
        }(self.videoDevice, self.exposureModeControl, self.exposureDurationSlider, self.ISOSlider);
    });
    ...
    //        AVCaptureFocusMode mode = (AVCaptureFocusMode)[self.focusModes[self.focusModeControl.selectedSegmentIndex] intValue];
    //        self.videoDevice.focusMode = mode;
    //    });
    //
    //    configureCameraSetting(self.videoDevice, ^ (void) {
    //        if ([self.videoDevice isTorchActive])
    //            [self.videoDevice setTorchMode:0];
    //    });
    //
    //    configureCameraSetting(self.videoDevice, ^ (void) {
    //        self.exposureModeControl.selectedSegmentIndex = 0;
...
- (IBAction)setPreset:(UIButton *)sender {
    dispatch_async( dispatch_get_main_queue(), ^{
        ...
        if (sender.tag == 1)
        {
            ...
            double maxDurationSeconds = 1.0/3.0;//CMTimeGetSeconds( self.videoDevice.activeFormat.maxExposureDuration );
            [self.videoDevice setExposureModeCustomWithDuration:CMTimeMakeWithSeconds( maxDurationSeconds, 1000*1000*1000 )  ISO:self.videoDevice.activeFormat.minISO completionHandler:nil];
